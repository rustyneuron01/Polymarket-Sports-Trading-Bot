...
Run from project root: `cd polymarket_nhl_bot` (or use full paths). Use `python -module.py>` or `python <script>.py` as shown.
...
```bash
python build_game_records.py --from 2026-02-01 --to 2026-02-26 --full-price-history
```
...
```bash
python build_game_records.py --from 2026-02-28 --to 2026-02-28 --snapshots-dir data/polymarket_snapshots/live_1s
```

Optional: fix only broken files, refresh prices, or remove incomplete:

```bash
python build_game_records.py --from 2026-02-01 --to 2026-02-26 --fix-invalid-only
python build_game_records.py --from 2026-02-01 --to 2026-02-26 --refresh-prices
python build_game_records.py --from 2026-02-01 --to 2026-02-26 --remove-event-feed-incomplete --output-dir data/game_records
```
...
```bash
python check_training_data.py --sample 20
...
```bash
python validate_game_records.py --dir data/game_records --sync-check
```
...
```bash
python generate_lineup_notes.py --fetch-espn --from 2026-02-01 --to 2026-02-26
...
```bash
python generate_lineup_notes.py --dir data/game_records --out data/lineup_notes.jsonl
```
...
# Or:
python train_model.py --dir data/game_records --model-out data/outcome_model.pkl --lineup data/lineup_notes.jsonl
...
```bash
python backtest_paper.py --days 14 --threshold 0.05 --stake 1.0 --out backtest_results
```
...
python data_collector.py --fast
python data_collector.py --interval 10
```
...
python fetch_game_outcomes.py --from 2025-10-01 --to 2026-02-26
python build_game_records.py --from 2025-10-01 --to 2026-02-26
...
# 6) Backtest
python backtest_paper.py --days 14
